...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $query = "SELECT m.judul, pr.nama AS provinsi, m.daerah, m.deskripsi, m.lirik, m.foto
              FROM musik m
              JOIN provinsi pr ON m.id_provinsi = pr.id_provinsi
              WHERE m.id_musik = $id";
...
<body>
    ...
    <div class="container">
        <h2>Song Details</h2>
        <?php if (!empty($song['foto'])) { ?>
            <img src="../img/<?php echo $song['foto']; ?>" alt="<?php echo $song['judul']; ?>" class="song-image">
        <?php } ?>
        <table>
            <tr>
                <th>Title</th>
                <td><?php echo $song['judul']; ?></td>
            </tr>
            <tr>
                <th>Province</th>
                <td><?php echo $song['provinsi']; ?></td>
            </tr>
            <tr>
                <th>Region</th>
                <td><?php echo $song['daerah']; ?></td>
            </tr>
            <tr>
                <th>Description</th>
                <td><?php echo $song['deskripsi']; ?></td>
            </tr>
            <tr>
                <th>Lyrics</th>
                <td><?php echo nl2br($song['lirik']); ?></td>
            </tr>
        </table>
        <a href="manage_lagu.php" class="btn btn-primary">Back to Manage Songs</a>
    </div>
</body>
</html>